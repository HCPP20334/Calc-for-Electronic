...
namespace ImGui {
    ...
    {
        return InputScalar(label, ImGuiDataType_S64, v, &step, &step_fast, NULL, flags);
    }
}
uint32_t fDataMemUsage() // Work Function !!! Check Sym RAM to Current Program //
{
    return 0;
}
ImVec4 clear_color = { 0,0,0,255 };
std::string BACKEND_API = "DX9 (Direct X 9.0)";
std::string BUILD_VER = "1.0.3(x86)";
#pragma once
// Data
...
int main(int, char**)
{
    ...
    HWND hwnd = ::CreateWindowW(wc.lpszClassName, L"Калькулятор электроника v 1.3.5 (Win32) WinXP_SP3", WS_OVERLAPPEDWINDOW, 100, 100, 800, 600, nullptr, nullptr, wc.hInstance, nullptr);
...
// Main loop
    ...
    while (!done)
    {
        ...
        ImGui::Begin("Калькулятор электроника", &fFr_main, ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoResize | ImGuiWindowFlags_NoMove);
        i64MaxFPS = i64MaxFPS;
        ...
        if (ELC_LaunchBool) {
            if (!fCAboutW) {//
               ...
               // ImGui::Image(GLArrayBuffer, ImVec2(198, 198));//GLArrayBuffer
                ...
                ImGui::SetCursorPosX((ImGui::GetWindowWidth() - 100 + 20) / 2);
                if (ImGui::Button("Debug", { 100,30 })) {
            ...
            if (fCAboutW) {
                ...
                ImGui::SetCursorPosX(15);
                if (ImGui::Button("Закрыть", { 340,30 })) {
                    fCAboutW = false;
                }
                //ImGui::OpenPopup("per_mode");
